...
def convert_roberta_checkpoint_to_tf(roberta_checkpoint_path, ckpt_dir, model_name):
    ...
    var_map = (
        ...
        ('lm_head.layer_norm.weight', 'cls/predictions/transform/LayerNorm/kernel'),
    ...
    with tf.Session() as session:
        state_dict = roberta.model.decoder.state_dict()
        ...
        torch_tensor = np.zeros((2, roberta.args.encoder_embed_dim))
        tf_var = create_tf_var(tensor=torch_tensor, name=tf_name, session=session)
        tf.keras.backend.set_value(tf_var, torch_tensor)
        tf_weight = session.run(tf_var)
        print("Successfully created {}: {}".format(tf_name, np.allclose(tf_weight, torch_tensor)))
